...
int NonNewton_Peer2016::ITERATIONS_V = -1;
int NonNewton_Peer2016::ITERATIONS_OMEGA = -1;
int NonNewton_Peer2016::MAX_ITERATIONS_V = -1;
int NonNewton_Peer2016::MAX_ERROR_V = -1;
int NonNewton_Peer2016::MAX_ITERATIONS_OMEGA = -1;
int NonNewton_Peer2016::MAX_ERROR_OMEGA = -1;


NonNewton_Peer2016::NonNewton_Peer2016(FluidModel *model) :
        ViscosityBase(model)
{
...
NonNewton_Peer2016::~NonNewton_Peer2016(void)
{
...
void NonNewton_Peer2016::initParameters()
{
...
void NonNewton_Peer2016::computeDensities()
{
...
void NonNewton_Peer2016::matrixVecProdV(const Real* vec, Real *result, void *userData)
{
    Simulation *sim = Simulation::getCurrent();
    NonNewton_Peer2016* visco = (NonNewton_Peer2016*)userData;
...
void NonNewton_Peer2016::diagonalMatrixElementV(const unsigned int i, Real &result, void *userData)
{
    // Diagonal element
    NonNewton_Peer2016* visco = (NonNewton_Peer2016*)userData;
    FluidModel* model = visco->getModel();
...
void NonNewton_Peer2016::matrixVecProdOmega(const Real* vec, Real *result, void *userData)
{
    Simulation *sim = Simulation::getCurrent();
...
void NonNewton_Peer2016::diagonalMatrixElementOmega(const unsigned int i, Real &result, void *userData)
{
    // Diagonal element
    Simulation *sim = Simulation::getCurrent();
    FluidModel *model = (FluidModel*)userData;
...
#pragma omp for schedule(static) nowait
        for (int i = 0; i < numParticles; i++)
        {
            ...
            const Real viscosity = static_cast<Real>(1.0) - m_viscosity_nonNewton[i];
...
#pragma omp for schedule(static) nowait
        for (int i = 0; i < (int)numParticles; i++)
        {
            ...
            g1[i] = omegai[1];
            g2[i] = omegai[2];
            b0[i] = m_viscosity_nonNewton[i] * rhs[0];
            b1[i] = m_viscosity_nonNewton[i] * rhs[1];
            b2[i] = m_viscosity_nonNewton[i] * rhs[2];
        }
    }
...
void NonNewton_Peer2016::reset()
{
}

void NonNewton_Peer2016::performNeighborhoodSearchSort()
{
}
