...
int NonNewton_standard::VISCOSITY_COEFFICIENT_BOUNDARY = -1;


NonNewton_standard::NonNewton_standard(FluidModel *model) :
        ViscosityBase(model)
{
...
NonNewton_standard::~NonNewton_standard(void)
{
}

void NonNewton_standard::initParameters()
{
...
void NonNewton_standard::step()
{
	...
	#pragma omp parallel default(shared)
	{
		...
		for (int i = 0; i < (int)numParticles; i++)
		{
            ...
            const Scalarf8 dvisc(d * m_viscosity_nonNewton[i] * density0);
            const Scalarf8 dbvisc(d * m_boundaryViscosity_nonNewton[i]);
 				...
 				else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017)
 				{
 					forall_density_maps(
						...
						if (nl > static_cast<Real>(0.0001))
						{
							...
							const Vector3r a1 = (d * m_boundaryViscosity_nonNewton[i] * vol * (vi - v1).dot(xix1) / (xix1.squaredNorm() + static_cast<Real>(0.01)*h2)) * gradW1;
							const Vector3r a2 = (d * m_boundaryViscosity_nonNewton[i] * vol * (vi - v2).dot(xix2) / (xix2.squaredNorm() + static_cast<Real>(0.01)*h2)) * gradW2;
							const Vector3r a3 = (d * m_boundaryViscosity_nonNewton[i] * vol * (vi - v3).dot(xix3) / (xix3.squaredNorm() + static_cast<Real>(0.01)*h2)) * gradW3;
							const Vector3r a4 = (d * m_boundaryViscosity_nonNewton[i] * vol * (vi - v4).dot(xix4) / (xix4.squaredNorm() + static_cast<Real>(0.01)*h2)) * gradW4;
							ai += a1 + a2 + a3 + a4;
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++)
        {
            ...
            const Vector3r xixj = xi - xj;
            ai += d * m_viscosity_nonNewton[i] * (model->getMass(neighborIndex) / density_j) * (vi - vj).dot(xixj) / (xixj.squaredNorm() + 0.01*h2) * sim->gradW(xi - xj);
            )
            ...
            if (m_boundaryViscosity_nonNewton[i] != 0.0)
            {
                if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012)
                {
                    ...
                    const Vector3r xixj = xi - xj;
                    const Vector3r a = d * m_boundaryViscosity_nonNewton[i] * (density0 * bm_neighbor->getVolume(neighborIndex) / density_i) * (vi - vj).dot(xixj) / (xixj.squaredNorm() + 0.01*h2) * sim->gradW(xi - xj);
                    ai += a;
                ...
                else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017)
                {
                    ...
                    if (nl > static_cast<Real>(0.0001))
                    {
                        ...
                        // compute forces for both sample point
                        const Vector3r a1 = d * m_boundaryViscosity_nonNewton[i] * vol * (vi - v1).dot(xix1) / (xix1.squaredNorm() + 0.01*h2) * gradW1;
                        const Vector3r a2 = d * m_boundaryViscosity_nonNewton[i] * vol * (vi - v2).dot(xix2) / (xix2.squaredNorm() + 0.01*h2) * gradW2;
                        const Vector3r a3 = d * m_boundaryViscosity_nonNewton[i] * vol * (vi - v3).dot(xix3) / (xix3.squaredNorm() + 0.01*h2) * gradW3;
                        const Vector3r a4 = d * m_boundaryViscosity_nonNewton[i] * vol * (vi - v4).dot(xix4) / (xix4.squaredNorm() + 0.01*h2) * gradW4;
                        ai += a1 + a2 + a3 + a4;
