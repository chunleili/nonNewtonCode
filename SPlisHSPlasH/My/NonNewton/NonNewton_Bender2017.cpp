#include "NonNewton_Bender2017.h"
...
int NonNewton_Bender2017::ITERATIONS = -1;
int NonNewton_Bender2017::MAX_ITERATIONS = -1;
int NonNewton_Bender2017::MAX_ERROR = -1;


NonNewton_Bender2017::NonNewton_Bender2017(FluidModel *model) :
        ViscosityBase(model)
{
...
NonNewton_Bender2017::~NonNewton_Bender2017(void)
{
...
void NonNewton_Bender2017::initParameters()
{
...
void NonNewton_Bender2017::step()
{
    ...
    #pragma omp for schedule(static)
        for (int i = 0; i < (int)numParticles; i++)
        {
            ...
            if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Akinci2012)
            {
                ...
                const Vector3r a = -invH * 0.1 * m_viscosity_nonNewton[i] * (density0 * bm_neighbor->getVolume(neighborIndex) / density_i) * (vi - vj)* sim->W(xi - xj);
                ai += a;
            ...
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Koschier2017)
            {
                ...
                const Vector3r a = -invH * 0.1 * m_viscosity_nonNewton[i] * (density0 / density_i) * (vi-vj)* rho;
                ai += a;
                bm_neighbor->addForce(xj, -m_model->getMass(i) * a);
                );
            }
            else if (sim->getBoundaryHandlingMethod() == BoundaryHandlingMethods::Bender2019)
            {
                ...
                const Vector3r a = -invH * 0.1 * m_viscosity_nonNewton[i] * (density0 * Vj / density_i) * (vi-vj)* sim->W(xi - xj);
                ai += a;
                bm_neighbor->addForce(xj, -m_model->getMass(i) * a);
                );
            }
...
void NonNewton_Bender2017::reset()
{
}


void NonNewton_Bender2017::computeViscosityFactor()
{
    //////////////////////////////////////////////////////////////////////////
    // Init parameters
    //////////////////////////////////////////////////////////////////////////
...
void NonNewton_Bender2017::computeTargetStrainRate()
{
    Simulation *sim = Simulation::getCurrent();
    const int numParticles = (int) m_model->numActiveParticles();
...
#pragma omp for schedule(static) nowait
        for (int i = 0; i < numParticles; i++)
        {
            ...
            const Real m = m_model->getMass(neighborIndex);
            const Real m2 = m * static_cast<Real>(2.0);
            strainRate[0] += (static_cast<Real>(1.0)-m_viscosity_nonNewton[i]) * m2 * vji[0] * gradW[0];
            strainRate[1] += (static_cast<Real>(1.0)-m_viscosity_nonNewton[i]) * m2 * vji[1] * gradW[1];
            strainRate[2] += (static_cast<Real>(1.0)-m_viscosity_nonNewton[i]) * m2 * vji[2] * gradW[2];
            strainRate[3] += (static_cast<Real>(1.0)-m_viscosity_nonNewton[i]) * m * (vji[0] * gradW[1] + vji[1] * gradW[0]);
            strainRate[4] += (static_cast<Real>(1.0)-m_viscosity_nonNewton[i]) * m * (vji[0] * gradW[2] + vji[2] * gradW[0]);
            strainRate[5] += (static_cast<Real>(1.0)-m_viscosity_nonNewton[i]) * m * (vji[1] * gradW[2] + vji[2] * gradW[1]);
            )
...
void NonNewton_Bender2017::performNeighborhoodSearchSort()
{
